...
class Detective:
    ...
    async def _run_live(self, repo_url: str, error_log: str) -> AgentResult:
        ...
        try:
            analysis = await asyncio.to_thread(reason, "deep", "You are Umbra Detective. Build a root-cause chain from the error log, verified local history, and Codex survey. Mention a commit SHA only if it is in the supplied history. Do not fabricate files, lines, or evidence.", f"Error log:\n{error_log}\nHistory:\n{history}\nCodex survey:\n{operation.summary}\nDiff:\n{operation.diff}", "xhigh")
