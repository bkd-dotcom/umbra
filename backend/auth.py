...
async def github_app_info():
    ...
    to install it. Lets the dashboard render (or hide) the install button."""
    return {"configured": github_app_configured(), "install_url": install_url()}


@router.get("/api/my/app-installations")
async def my_app_installations(request: Request):
    """Installations the signed-in user has linked (via the App setup redirect),
    with the repos each one auto-reviews. Only ever the caller's own records."""
    user = get_current_user(request)
    return get_store().list_installations_for_user(_user_key(user))
