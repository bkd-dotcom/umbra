...
deterministic ``VerifierReport`` — offline, no model, no Codex.
...
- **advisory_cleared**: when a CVE + fixed version are claimed, the bumped
  manifest content actually pins a version that escapes the advisory range
  (reuses the same OSV math the bump PR uses — no trust in the agent's word).
- **tests**: records the declared test command + exit code (``passed`` /
  ``failed`` / ``unavailable``) — never invents a pass.
- **citations**: every changed path the change claims is present in the changeset
  (no phantom files).

The report yields ``status`` ∈ {reviewable, blocked} and an
``evidence_completeness`` percentage — how much of the ideal evidence bundle is
actually present — so a reviewer sees *what could not be verified*, not a false
green. ``blocked`` means a hard safety check failed (contract or secret); missing
soft evidence (no test command) lowers completeness but stays ``reviewable``.
"""
...
from backend.contract import ContractResult
from backend.features import scan_secrets
from backend.remediation import _version_key
...
class VerifierCheck:
    ...
    detail: str
    blocking: bool = False  # a failing blocking check forces status="blocked"
...
class VerifierReport:
    ...
    evidence_completeness: int = 0  # 0-100
...
def _advisory_cleared(new_manifest: str, package: str, fixed: str) -> tuple[bool, str]:
    ...
    Deterministic string/version check — we don't trust that the agent claimed a
    fix; we read the pinned version out of the produced manifest and compare it to
    the required fixed version using the same version-key ordering OSV math uses.
    """
    import re

    # npm ("pkg": "^1.2.3") or PyPI (pkg==1.2.3) pin, tolerant of range operators.
    ...
    if _version_key(pinned) >= _version_key(fixed):
...
def verify_change(
    ...
    *,
    ...
    ``file_changes`` maps changed path → new file content. ``contract_result`` is
    the verdict from :func:`backend.contract.evaluate_contract`. The advisory and
    test parameters are optional context; when absent the relevant check is
    ``unavailable`` (lowering completeness) rather than a fabricated pass.
    """
    checks: list[VerifierCheck] = []
    changed = sorted(file_changes.keys())

    # 1. Contract compliance (blocking) — reuse the deterministic contract verdict.
    ...
    # 2. Secret scan (blocking) — no credential may be introduced in new content.
    ...
    # 3. Advisory cleared (soft) — only when a CVE + fix are claimed for a package.
    ...
    # 4. Tests (soft) — record the declared command + exit code; never invent a pass.
    ...
    # 5. Citations (soft) — every claimed changed file is present in the changeset.
    if claimed_files:
    ...
    # Evidence completeness: fraction of checks that produced a real (non-unavailable)
    # verdict — i.e. how much of the ideal evidence bundle is actually present.
    resolved = sum(1 for c in checks if c.status != "unavailable")
    completeness = round(100 * resolved / len(checks)) if checks else 0

    # Status: blocked iff any BLOCKING check failed. Missing soft evidence keeps it
    # reviewable (with lower completeness) — a reviewer decides, Umbra never fakes a pass.
