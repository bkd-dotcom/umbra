  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="h-[300px] w-full">
      ...
      {shown.map((d, i) => {
        ...
        return (
          <motion.g key={`n-${d.name}-${i}`} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.15 + i * 0.02, type: "spring", stiffness: 260, damping: 18 }}>
            <circle cx={x} cy={y} r={d.vulnerable ? 5 : 3.5} fill={color} style={d.vulnerable ? { filter: "drop-shadow(0 0 6px #fb7185)" } : undefined} />
          </motion.g>
        );
      })}
      <motion.circle cx={c} cy={c} r={16} fill="#0a0c14" stroke="#22d3ee" strokeWidth={1.5} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 220, damping: 16 }} style={{ filter: "drop-shadow(0 0 10px #22d3ee66)" }} />
